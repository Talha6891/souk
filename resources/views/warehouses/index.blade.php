...
@section('content')
    {{-- breadcrumbs start --}}
    <x-page-title title="Warehouses List" :breadcrumbs="$breadcrumbs" />
    {{-- breadcrumb end --}}
        ...
        <div class="card-body">
            ...
            <div class="flex flex-col items-start justify-between gap-5 mb-5 md:flex-row md:items-center">
                <div class="relative md:w-3/6">
                    <form id="searchForm" method="get" action="{{ route('warehouses.index') }}">
                ...
                @can('warehouse create')
                    <div class="flex space-x-2 md:ml-auto">
                        <a href="{{ route('warehouses.create') }}"
                            ...
                            id="create-btn">
                            <i class="align-bottom ri-add-line me-1"></i> Add Warehouse
                        </a>
                    </div>
                @endcan
            </div>
                        ...
                        @forelse($warehouses as $warehouse)
                            <tr>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                    {{ Str::limit($warehouse->id, 20) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                    {{ Str::limit($warehouse->name, 20) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 email">
                                    {{ Str::limit($warehouse->email, 20) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                    {{ Str::limit($warehouse->contact_no, 15) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                    {{ Str::limit($warehouse->user->first_name . ' ' . $warehouse->user->last_name, 15) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                    {{ Str::limit($warehouse->country->name, 12) }}</td>
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 ">
                                ...
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                    <div class="flex gap-2">
                                        ...
                                        @can('user delete')
                                            <div class="remove">
                                                <form id="deleteForm{{ $warehouse->id }}" method="POST"
                                                    action="{{ route('users.destroy', $warehouse) }}"
                                                    ...
                                                    <a class="cursor-pointer action-btn"
                                                        onclick="sweetAlertDelete(event, 'deleteForm{{ $warehouse->id }}')">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="flex justify-end mt-4">
                <div class="flex gap-2 pagination-wrap">
                    {{ $warehouses->onEachSide(1)->links('pagination.custom-tailwind') }}
                </div>
